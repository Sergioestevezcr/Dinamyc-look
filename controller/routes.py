...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if 'loggedin' not in session or session.get('rol', '').lower() != 'administrador':
...
def index_admin():
    ...
    cur.execute(
        'SELECT COUNT(ID_Venta) + 1 AS Ventas_Totales, Sum(Total) AS Ingresos_Totales FROM ventas')
    ...
    return render_template('Vista_admin/index-admin.html',
                           ventas=ventas,
                           ingresos=ingresos,
...
def productos():
    ...
    return render_template('vista_admin/productos.html', productos=data)
...
def usuarios():
    ...
    return render_template('vista_admin/usuarios.html', usuarios=usuarios_data)
...
def add_usuario():
    if request.method == 'POST':
        ...
        user.add_user(nombre, apellido, correo,
                      telefono, direccion, ciudad, clave)
...
def update_usuario(id):
    if request.method == 'POST':
        ...
        user.update_user(id, nombre, apellido, correo,
                         telefono, direccion, ciudad, clave)
...
def ventas():
    ...
    return render_template('vista_admin/ventas.html',
...
def add_venta():
    if request.method == 'POST':
        ...
        if not productos_data or not cantidades:
            ...
            # Podrías redirigir a otra página de usuario si la venta no es en la vista admin
            return redirect(url_for('main.ventas'))

        productos_venta = []
        for i, producto_id in enumerate(productos_data):
            if producto_id and int(cantidades[i]) > 0:
                producto_info = product.get_product_price_and_stock(
                    producto_id)
                if producto_info:
                    ...
                    nombre_producto = producto_completo[
                        1] if producto_completo else f"Producto ID {producto_id}"

                    if cantidad_solicitada > stock_disponible:
                        flash(
                            f'Stock insuficiente para {nombre_producto}. Disponible: {stock_disponible}', 'warning')
                        # Podrías redirigir a una página de usuario
                        return redirect(url_for('main.ventas'))
...
# La factura debe ser accesible por el usuario que realizó la compra o un admin
@login_required
def factura(id_venta):
    ...
    return render_template('vista_admin/factura.html',  # Considera tener una plantilla de factura separada para usuarios si la vista de admin es muy diferente
